...
const handleStep1 = msg => {
  ...
  bot.sendMessage(
    chatId,
    `_${'Для того, чтобы добавить открытую позицию выполните несколько действий'}_.\n\n*${'Шаг 1: Укажите пару'}*`,
    options
  )
  
  bot.onText(/\/pepe/, txt => {
    callbackQuery.data = txt
  ...
  if (check === true) {
    ...
    bot.sendMessage(chatId, `_${'Отправлено в канал:'}_`, parseMarkdown).then(() => {
      if (screenshots.length > 0) {
        ...
        const channelId = '-1001904496260' // ID of my crypto trading channel
...
bot.onText(/\/pv/, msg => {
	...
	bot.on('message', callbackQuery => {
		if (count < 1) {
			...
			const calculatePosVol = (risk, sl) => {
				return posVol = risk / (sl / 100)
			}
			...
			const positionVolume = roundUp(calculatePosVol(risk, sl), 2)
			const leverage = roundUp((positionVolume * 0.65) / risk, 0)
			const margin = roundUp(positionVolume / leverage, 4)
			const percSL = roundUp(sl * leverage, 2)
			const percTP = roundUp(percSL * ratio, 2)

			count++
			bot.sendMessage(chatId, `Номинальный объём: *${ positionVolume }*\nПлечо: *${ leverage + 'x' }*\nМаржа: *${ '$' + margin }*\nСтоп-лосс: *${ percSL + '%' }*\nТейк-профит: *${ percTP + '%' }*`, parseMarkdown)
		}
	})
